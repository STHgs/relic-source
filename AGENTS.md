...
- [x] Claude Code 适配器退役（2026-09-15，用户拍板）：src/adapters/claude.mjs 删除，DEFAULT_ADAPTERS 三适配器，paths/测试全部摘除；现网 ~/.claude/AGENTS.md 保留最后版本不再更新（该平台未部署使用）。289 tests 287 pass。
- [x] Windows 原生适配（2026-09-15）：exec.mjs 平台无关执行层（数组化+win32 shell+headless env+runChain+userHome 归一）；install-schedule 三平台分支（schtasks/launchd 补欠账/systemd→cron）；paths.mjs 平台路径变体 + opencode 双路径探测（AppData 优先待实测收敛）；四适配器接入；E 盘 --content 真 sync 实测 Windows 盘写入链路；295 tests 293 pass。
